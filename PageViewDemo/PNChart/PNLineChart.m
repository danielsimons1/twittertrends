...
- (void)touchPoint:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    for (NSInteger p = _pathPoints.count - 1; p >= 0; p--) {
        ...
        if (linePointsArray && linePointsArray.count > 0) {
    ...
    for (NSUInteger lineIndex = 0; lineIndex < self.chartData.count; lineIndex++) {
        ...
        for (NSUInteger i = 0; i < chartData.itemCount; i++) {
            ...
            if (!(_yValueMax - _yValueMin)) {
                ...
                innerGrade = (yValue - _yValueMin) / (_yValueMax - _yValueMin) == 0 ? (yValue + 1 - _yValueMin) / (_yValueMax - _yValueMin) : (yValue - _yValueMin) / (_yValueMax - _yValueMin);
            }
